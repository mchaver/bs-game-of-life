...
function drawGrid(canvas) {
  ...
  var width = 2;
  while(((width + 2 | 0) + 18 | 0) < maxWidth) {
    context.moveTo(width, 0);
    context.lineTo(width, maxHeight);
    width = (width + 2 | 0) + 18 | 0;
  };
  var height = 1;
  while(((height + 2 | 0) + 18 | 0) < maxHeight) {
  ...
  context.stroke();
  context.fillStyle = foregroundColor;
  context.fillRect(3, 2, 18, 18);
  context.fillRect(23, 2, 18, 18);
  return /* () */0;
}

function runonce(canvas) {
  var context = canvas.getContext("2d");
  context.lineWidth = 2.0;
  context.strokeStyle = gridColor;
  context.beginPath();
  context.moveTo(15.0, 200.0);
  context.lineTo(0.0, 200.0);
  context.stroke();
  return /* () */0;
}
...
var rows$1 = canvas.width / 8 | 0;

var columns$1 = canvas.height / 8 | 0;

Random.self_init(/* () */0);

$$Array.map((function (cells) {
